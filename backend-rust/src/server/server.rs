...
use crate::private_service::{PrivateServiceConfig, ServiceCredential, run_private_service};
...
impl Server {
    pub async fn new(config: Config) -> Result<()> {
        ...
        let private_service = config.private_service.clone();
        let secret_key = config.session_secret_key.0.clone();
        let config_data = web::Data::new(config);
        let private_vault_config = config_data.get_ref().clone();
        ...
        if private_service.enabled {
            let bind = format!("{}:{}", private_service.listen, private_service.port)
                .parse()
                .map_err(|_| anyhow::anyhow!("invalid private service bind address"))?;
            let private_config = PrivateServiceConfig::new(
                bind,
                ServiceCredential::new(private_service.credential),
                private_service.allow_container_bind,
            )?;
            let (_, _) = tokio::try_join!(
                public_server,
                run_private_service(private_config, private_vault_config),
            ).map_err(anyhow::Error::new)?;
            Ok(())
        } else {
            public_server.await.map_err(anyhow::Error::new)
        }
    }
}
